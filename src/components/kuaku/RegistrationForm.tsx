...
// Pastikan path ini benar menunjuk ke Server Action Anda
import { submitRegistrationForm, type RegistrationFormState } from "@/app/pendaftaran/actions"; 
import { useToast } from "@/hooks/use-toast"; // Pastikan Anda memiliki hook ini

// Sesuaikan skema ini dengan Server Action Anda
...
  dateOfBirth: z.date({ required_error: "Tanggal lahir wajib diisi." }).optional(),
...
// Ganti nama 'RegistrationForm' jika ini adalah komponen default halaman
...
  });
  ...
  // Ini adalah "Aksi" yang Anda cari (redirect setelah sukses)
  useEffect(() => {
    if (state.message) {
...
                </PopoverContent>
              </Popover>
              {/* Input tersembunyi ini penting untuk mengirim data tanggal ke Server Action */}
