...
const navItems = [
  { href: '/admin/staff', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/admin/staff/pendaftaran', label: 'Pendaftaran Nikah', icon: UserPlus },
  { href: '/admin/staff/pengumuman', label: 'Pengumuman Nikah', icon: FileCheck },
      ...
      >
        <div>
          <div className={cn("flex items-center border-b", isCollapsed ? "justify-center h-16" : "justify-between h-16 px-4")}>
            {!isCollapsed && (
              <Link href="/admin/staff" className="flex items-center gap-2">
                ...
                <span className="font-bold text-lg">Staff KUA</span>
              </Link>
            )}
            <Button
              variant="ghost"
          ...
          <nav className={cn("flex flex-col gap-1 px-2", isCollapsed ? "py-4 items-center" : "py-4")}>
            {navItems.map((item) => {
              const isActive = pathname === item.href || (item.href === '/admin/staff' && pathname.startsWith('/admin/staff') && pathname !== '/admin/staff/pendaftaran');
              return isCollapsed ? (
          ...
          {isCollapsed ? (
            ...
            <>
              <div className="px-2 py-2">
                <p className="text-sm font-semibold truncate">{user?.nama || 'Staff KUA'}</p>
                <p className="text-xs text-muted-foreground">Staff KUA Banjarmasin Utara</p>
              </div>
